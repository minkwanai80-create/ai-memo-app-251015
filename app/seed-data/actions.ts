// app/seed-data/actions.ts
// 목업 데이터 생성 Server Actions
// 테스트용 샘플 노트 생성
// 관련 파일: app/seed-data/page.tsx
...
- WebP 포맷 사용`
    ...
    {
        title: '오늘의 일기',
        content: `날씨가 정말 좋은 하루였다. 아침에 일찍 일어나서 산책을 다녀왔는데, 상쾌한 공기가 기분을 좋게 만들어주었다. 

점심에는 새로 오픈한 이탈리안 레스토랑에 가봤는데, 파스타가 정말 맛있었다. 특히 까르보나라가 일품이었다.

오후에는 책을 읽으며 여유로운 시간을 보냈다. 요즘 읽고 있는 SF 소설이 너무 재미있어서 시간 가는 줄 모르겠다.
...
export async function seedMockNotes() {
    try {
        const supabase = await createClient()
        const {
            data: { user }
